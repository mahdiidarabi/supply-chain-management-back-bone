	...
	try {
		...
		if (response_payloads) {
			for (let i = 0; i < response_payloads.length; i++) {
				logger.info('query response is ' + response_payloads[i].toString('utf8'));
			}
			if (response_payloads[0].status == 500){
				...
				var response = {
					success : false ,
					message : response_payloads[0].message,
					metadata : null
				}
				return response;
			}
				...
				var response = {
					success : true ,
					message : "get query response successfully",
					metadata : response_payloads[0].toString('utf8')
				}
				return response;
			}
